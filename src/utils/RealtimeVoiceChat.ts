...
export class RealtimeVoiceChat {
  ...
  async init(instructions?: string, voice?: string) {
    try {
      ...
      const tokenResponse = await fetch('/functions/v1/openai-realtime-session', {
        method: 'POST',
        headers: {
      ...
      if (!sessionData.client_secret?.value) {
        throw new Error("Failed to get ephemeral token from session data");
      }
      ...
      // Start recording audio and sending to AI
