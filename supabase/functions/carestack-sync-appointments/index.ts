...
serve(async (req) => {
  ...
  try {
    ...
    // Sync appointments
    const syncResult = await adapter.syncAppointments(modifiedSince, continueToken || undefined)

    // Log the sync operation
    await supabase.from('audit_log').insert({
      clinic_id: authContext.clinic_id,
      user_id: authContext.user_id,
      action: 'carestack_sync_appointments',
      details: {
        office_id: officeId,
        modified_since: modifiedSince,
        appointment_count: syncResult.items.length,
