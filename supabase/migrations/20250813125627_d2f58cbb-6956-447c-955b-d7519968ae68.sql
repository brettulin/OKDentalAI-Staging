-- Fix the validate_production_readiness function with proper search path
DROP FUNCTION IF EXISTS public.validate_production_readiness(uuid);
...
BEGIN
  ...
  WHERE clinic_id = p_clinic_id AND started_at > now() - interval '7 days' 
